...
export const GuruSidebar: React.FC<GuruSidebarProps> = ({ onCloseMobileMenu }) => {
  ...
  const initials = useMemo(() => {
    if (!user?.nama && !user?.name) return 'GP';
  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        {
          href: '/guru/jurnal',
          label: 'Validasi Jurnal & Absensi',
          icon: CheckSquare,
          badge: jurnal_perlu_evaluasi,
        ...
        },
        {
          ...
          icon: Users,
          badge: total_siswa_bimbingan,
          badgeColor: 'bg-blue-100 text-blue-800',
        },
        { href: '/guru/kunjungan', label: 'Kunjungan Lapangan', icon: MapPin },
      ],
    },
  ];
                ...
                return (
                  ...
                  >
                    ...
                    {item.badge !== undefined && item.badge > 0 && (
                      <span className={`inline-flex items-center justify-center px-2 py-0.5 text-[10px] font-extrabold rounded-full shrink-0 ${item.badgeColor}`}>
                        {item.badge}
                      </span>
                    )}
                  </Link>
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <p
                ...
                title={user?.nama || user?.name || 'Guru Pembimbing'}
                suppressHydrationWarning
              >
                {user?.nama || user?.name || 'Guru Pembimbing'}
              </p>
              <p
